...
from utils.callbacks import GenerateEveryNSteps_Arbitrary_Source
...
def main(config):
    ...
    test_source_dir = os.path.join(results_dir, 'test_source_meshes')
    test_source_v = train_dataset.sample_random_sources(config.evaluate.num_samples, saving_dir=test_source_dir, device='cuda')
    ...
    callbacks = [
        ...
        GenerateEveryNSteps_Arbitrary_Source(
    ...
    model.generate_arbitrary_source(test_source_v, deformed_meshes_dir, 
    ...
    if not config.get("render_results", False):  
        ...
        test_source_videos = os.path.join(results_dir, "test_source_videos")
        render_trajectory(test_source_dir, test_source_videos,
                        R=trajectory_configs.R, h=trajectory_configs.h, N=trajectory_configs.N)
        ...
        if not disable_wandb:
            ...
            eval_source_paths = sorted(os.listdir(test_source_videos))
            wandb.log({"source_mesh_videos": [
                        wandb.Video(os.path.join(test_source_videos, i)) for i in eval_source_paths
                    ]
                }
            )
